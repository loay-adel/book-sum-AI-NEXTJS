...
const MainContent = () => {
  ...
  const handleBookSelect = (bookTitle) => {
    ...
    setActiveTab("search");
    // Auto-search when selecting a book
    setTimeout(() => {
  ...
  const clearResults = () => {
    // This would require modifying hooks to include reset functions
    // For now, we'll just reset the search query and selected file
    setSearchQuery("");
              ...
              >
                ...
                {!searchResults && !searchLoading && (
                  ...
                  >
                    ...
                    <div>
                      <h3 className="text-xl font-semibold text-white mb-4">
                        {dict[lang].trendingNow}
                      </h3>
                      <div className="space-y-3">
                        {categoryBooks.slice(0, 3).map((book, index) => (
                          <div
                            key={index}
                            className="p-3 bg-gray-800 rounded-lg hover:bg-gray-700 transition-colors cursor-pointer flex items-center gap-3"
                            onClick={() => handleBookSelect(book.title)}
                          >
